...
llm_with_tools = llm.bind_tools(get_financial_tools())

def estimate_context_tokens(messages) -> int:
    """Helper function to roughly estimate token count of messages."""
...
def orchestrator(state: AgentState):
    ...
    if not state.get("messages"):
        ...
        # Fixed: Removed the forced 'search_child_chunks' since the tool does not exist
        response = llm_with_tools.invoke([sys_msg] + summary_injection + [human_msg])
    ...
    for msg in reversed(messages):
        if isinstance(msg, AIMessage) and getattr(msg, "tool_calls", None):
            for tc in msg.tool_calls:
                # Track tool usage for context compression (using your current tools)
                if tc["name"] in ["search_financial_tables", "search_unstructured_text"]:

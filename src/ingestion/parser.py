        ...
        if df_list:
            ...
            if len(df.columns) > 1:
                while len(df) > 0:
                    ...
                    first_cell_empty = pd.isna(first_cell) or str(
                        first_cell
                    ).strip() in ("", "nan", "None")
            ...
            if not is_col_i:
                ...
                for i in range(len(df) - 1):
                    ...
                    if pd.notna(df.iloc[i, 0]) and df_snapshot.iloc[i, 1:].isna().all():
                        ...
                        df.iloc[i + 1, 0] = (
                            f"{prefix_text} {str(df.iloc[i + 1, 0]).strip()}"
                        )
            ...
            for i in range(len(df)):
                ...
                rest_meaningful = rest_meaningful[
                    ~rest_meaningful.isin(("", first_col_val))
                ]
            ...
            for col_idx in range(1, len(df.columns)):
                ...
                non_null_mask = col_vals.notna() & (
                    col_vals.astype(str).str.strip() != ""
                )
...
def process_all_raw_html():
    ...
    for company in companies:
        ...
        years = [
            y
            for y in os.listdir(year_path)
            if os.path.isdir(os.path.join(year_path, y))
        ]
        ...
        for year in years:
            ...
            for report in reports:
                ...
                parse_10k_html(
                    file_path,
                    os.path.join(BASE_DIR, f"data/processed/{company}/{year}/"),
                )
